...
class RandomWeightedAverage(_Merge):
    ...
    """Provides a (random) weighted average"""
...
class MuseGAN():
    ...
    , using_generator = False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if epoch % print_every_n_batches == 0:
                ...
                self.generator.save_weights(os.path.join(run_folder, 'weights_record/weights-g.h5'))
                
                self.critic.save_weights(os.path.join(run_folder, 'weights_record/weights-c.h5'))
            ...
            if epoch % 500 == 0:
                self.generator.save_weights(os.path.join(run_folder, 'weights_record/weights-g-%d.h5' % (epoch)))
                self.critic.save_weights(os.path.join(run_folder, 'weights_record/weights-c-%d.h5' % (epoch)))
    ...
    def notes_to_midi(self, run_folder, output, filename = None):
        ...
        for score_num in range(len(output)):
            ...
            if filename is None:
                parts.write('midi', fp=os.path.join(run_folder, "data_storage/sample_{}_{}.midi".format(self.epoch, score_num)))
            else:
                parts.write('midi', fp=os.path.join(run_folder, "data_storage/{}.midi".format(filename)))


    def plot_model(self, run_folder):
        plot_model(self.model, to_file=os.path.join(run_folder ,'vizualizations/model.png'), show_shapes = True, show_layer_names = True)
        plot_model(self.critic, to_file=os.path.join(run_folder ,'vizualizations/critic.png'), show_shapes = True, show_layer_names = True)
        plot_model(self.generator, to_file=os.path.join(run_folder ,'vizualizations/generator.png'), show_shapes = True, show_layer_names = True)
    ...
    def load_weights(self, run_folder, epoch=None):
        ...
        if epoch is None:
            ...
            self.generator.load_weights(os.path.join(run_folder, 'weights_record', 'weights-g.h5'))
            self.critic.load_weights(os.path.join(run_folder, 'weights_record', 'weights-c.h5'))
        else:
            self.generator.load_weights(os.path.join(run_folder, 'weights_record', 'weights-g-{}.h5'.format(epoch)))
            self.critic.load_weights(os.path.join(run_folder, 'weights_record', 'weights-c-{}.h5'.format(epoch)))
    ...
    def draw_score(self, data, score_num):
        ...
        fig, axes = plt.subplots(ncols=self.n_bars, nrows=self.n_tracks,figsize=(7776,8882), sharey = True, sharex = True)
